...
import { PRIZE_SHEET, type PrizeTier } from "./prize-sheet";
...
export async function recordPrizeTierUnlocks(
  ...
  void notifyUserPrizeUnlock(userId, newlyUnlocked);
}

/** Web push + in-app queue when user crosses a prize tier. */
    ...
    for (const sub of subs || []) {
      ...
      if (result.ok) {
        await db
          ...
          .eq("id", sub.id);
      } else if ("statusCode" in result && (result.statusCode === 404 || result.statusCode === 410)) {
        await db
          .from("push_subscriptions")
          .update({
            is_active: false,
